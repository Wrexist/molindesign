...
function otherSnippet(meta, url) {
  ...
  if (meta.mode === 'sequence') {
    ...
    if (meta.webm) lines.push('<!-- video with alpha (Chrome, Edge, Firefox); Safari: use the animated WebP or an MP4 -->', `<video src="${attr(url + meta.webm)}" width="${f.width}" height="${f.height}" autoplay muted loop playsinline></video>`);
