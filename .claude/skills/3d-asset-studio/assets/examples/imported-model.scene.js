...
import {loadModel, describe, measure} from 'w3d/models.js';
...
export default async function build(ctx) {
  const set = await loadModel(new URL('./models/desk-set.glb', import.meta.url).href, {
    height: '27cm', by: /pot|soil|leaf/,  // the plant (its parts by name) is 27 cm; everything else scales with it
  ...
  ctx.log('plant ' + JSON.stringify(measure(set, /pot|soil|leaf/)) + ' cm');  // check, do not assume
